...
# discard()：删除集合中的指定元素，如果元素不存在，不会报错
s2 = {1, 2, 3, 4}
print('原集合：', s2)
s2.discard(2)
s2.discard(5) # 不报错，不进行任何操作
print('现集合：', s2)


# 4.交集和并集
#   1. 交集：两个集合中相同的元素，共有的部分
# &：取交集
a = {1, 2, 3, 4}
b = {3, 4, 5, 6}
print(a & b) # {3, 4}
c = {5, 6, 7, 8}
print(a & c) # set() --没有交集返回空集合set()

s1 = {'a', 'b'}
s2 = {'c', 'd'}
print(s1 & s2) # set() --没有交集返回空集合set()

#   2. 并集：两个集合中所有的元素，重复元素只保留一个
# |：取并集
a = {1, 2, 3, 4}
b = {3, 4, 5, 6}
